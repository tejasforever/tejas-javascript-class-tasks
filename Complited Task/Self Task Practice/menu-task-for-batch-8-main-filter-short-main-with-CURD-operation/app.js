...
function addToCart(id) {
  ...
  const cartItems = myCart.map(function (v) {
    return `<article class="cart-item carousel-item active">
...
// let tp = arry.push(123,12);
// let tp = arry.unshift(123,45,96);
let tp = arry.pop();
console.log(tp);
console.log(arry);
...
function pushMenuItem() {
  ...
  document.getElementById("btn").innerHTML = buttons.join(" ");
  console.log("Updated Categories:", newMenu);
}

function unshiftMenuItem() {
  if(!newMenu.includes("HT")){
    newMenu.unshift("HT");
  console.log(newMenu);
  }
  const buttons = newMenu.map(function (category) {
...
function spliceMenuItem() {
  // newMenu.splice(2,1);
  newMenu.splice(2,1,"Hello","How","Are");
